...
var g_chartContainerIDs = ["graphContainer", "barChartContainerWrapper"]; // pieChart rausgeschmissen
var g_chartContainerIDs = ["graphChartContainerWrapper", "barChartContainerWrapper"]; // pieChart rausgeschmissen
var g_chartIndex = 0;
...
// Setzt alle Klassen in ChartContainer zurueck
// um zu vermeiden, dass sich Charts ueberlappen
// nachdem ein anderes Bundesland ausgewaehlt wurde
function ResetChartContainer() {
 // eigentlich muessen nur alle Charts bis auf das erste zurueckgesetzt werden,
 // da sonst moeglicherweise die Animation unterbrochen wird
 for(var i = 1; i < g_chartContainerIDs.length; i++) {
   //document.getElementById(g_chartContainerIDs[i]).className = "bk-root";
 }
  g_chartIndex = 0;
...
function GetNextChartIndexRight(currentChartIndex, chartIdArr) {
  if (currentChartIndex <= chartIdArr.length -2) {
...
function GetNextChartIndexLeft(currentChartIndex, chartIdArr) {
  if (currentChartIndex > 0) {
    return currentChartIndex - 1;
  }
  else {
    return chartIdArr.length -1;
  }
}
...
function ShowChart(land)
{
    ...
    if(land != "Ganz Deutschland")
    {
        ...
        document.getElementById("ChartContainer").className = "I ChartEinblenden"; 
        document.getElementById("ChartControl").className = "";


        // Chart anzeigen
        //g_chartIndex = 1; // so als ob barchart selektiert waere
        document.getElementById(g_chartContainerIDs[0]).className = "bk-root Rechts0";
        document.getElementById(g_chartContainerIDs[1]).className = "bk-root O";
        //document.getElementById(g_chartContainerIDs[2]).className = "bk-root O";
    ...
    else
    {
        ...
        document.getElementById("ChartControl").className = "O";
